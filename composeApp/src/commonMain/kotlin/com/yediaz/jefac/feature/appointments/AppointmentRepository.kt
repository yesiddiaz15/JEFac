...
class AppointmentRepository {
    ...
    suspend fun createAppointment(
        ...
        return try {
            ...
            val response = supabase.postgrest["appointments"]
                .insert(
                    buildJsonObject {
                        put("business_id", appointment.businessId)
                        put("client_id", appointment.clientId)
                        put("service_id", appointment.serviceId)
                        if (appointment.professionalId != null)
                            put("professional_id", appointment.professionalId)
                        put("scheduled_at", appointment.scheduledAt)
                        put("status", appointment.status)
                        put("base_price", appointment.basePrice)
                        if (appointment.discountType != null)
                            put("discount_type", appointment.discountType)
                        put("discount_value", appointment.discountValue)
                        put("final_price", appointment.finalPrice)
                        put("commission_pct", appointment.commissionPct)
                        put("professional_earn", appointment.professionalEarn)
                        put("business_earn", appointment.businessEarn)
                        put("has_courtesy_drink", appointment.hasCourtesyDrink)
                        if (appointment.courtesyDrinkId != null)
                            put("courtesy_drink_id", appointment.courtesyDrinkId)
                        put("courtesy_cost", appointment.courtesyCost)
    ...
    suspend fun createClient(client: Client): Result<Client> {
        return try {
            val response = supabase.postgrest["clients"]
                .insert(
                    buildJsonObject {
                        put("business_id", client.businessId)
    ...
    private suspend fun registerAppointmentTransaction(appointment: Appointment): String? {
        return try {
            ...
            val category = when {
                appointment.serviceId.isNotEmpty() -> "nail_spa"
                else -> "other"
            }
            ...
            if (appointment.professionalEarn > 0) {
                supabase.postgrest["transactions"].insert(
                    buildJsonObject {
                        ...
                        put("amount", appointment.professionalEarn)
    ...
    private suspend fun registerCourtesyExpense(
        ...
        try {
            ...
            if (appointment is Result.Success) {
                supabase.postgrest["transactions"].insert(
                    buildJsonObject {
                        put("business_id", appointment.data.businessId)

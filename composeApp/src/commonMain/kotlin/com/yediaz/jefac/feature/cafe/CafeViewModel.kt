...
class CafeViewModel(
    ...
    private fun loadTables() {
        viewModelScope.launch {
            ...
            val ordersMap = if (ordersResult is Result.Success) {
                ordersResult.data.filter { it.tableId != null }.associateBy { it.tableId!! }
            } else emptyMap()
    ...
    private fun openTable(table: CafeTable) {
        viewModelScope.launch {
    ...
    private fun openAppointmentOrder(appointment: ActiveAppointmentUi) {
        viewModelScope.launch {
            _effects.send(CafeEffect.NavigateToOrder(
                tableId       = null,
                tableNumber   = 0,
